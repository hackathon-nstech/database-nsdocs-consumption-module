...
async function bootstrap() {
	...
	const maxReconnectAttempts = configService.get<number>('MAX_RECONNECT_ATTEMPTS') || 5;
		...
		try {
			...
			app.connectMicroservice<MicroserviceOptions>({
				...
				options: {
					...
					queue: configService.get<string>('DOCUMENTS_QUEUE') || 'documents_queue',
					queueOptions: {
						durable: true,
					},
			...
			console.error(`Failed to connect to RabbitMQ: ${error && error["message"]}`);

			reconnectAttempts++;
			if (reconnectAttempts < maxReconnectAttempts) {
				console.log(`Connection to transport failed. Trying to reconnect... Attempt ${reconnectAttempts}/${maxReconnectAttempts}`);
				setTimeout(connectToRabbitMQ, reconnectDelay);
			} else {
				console.error(`Failed to connect to RabbitMQ after ${maxReconnectAttempts} attempts. Giving up.`);
